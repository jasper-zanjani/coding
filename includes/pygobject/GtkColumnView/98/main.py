...
enterprise = Starship(name="USS Enterprise", registry = "NCC-1701", franchise= "TOS")
defiant = Starship("USS Defiant", "NX-74205", "DS9")
...
class MyApp(Adw.Application):
    def do_activate(self):
        ...
        #     ship = Starship(name = ship_name, registry = ship_info[0], franchise = ship_info[1])
        #     print(ship)
        #     data_model.append(ship)
        # data_model.append(enterprise)
        data_model.append(defiant)
        ...
        col1.get_factory().connect("setup", self._on_factory_setup)
        col1.get_factory().connect("bind", self._on_factory_bind, "title")
        col2.get_factory().connect("setup", self._on_factory_setup)
        col2.get_factory().connect("bind", self._on_factory_bind, "author")
        col3.get_factory().connect("setup", self._on_factory_setup)
        col3.get_factory().connect("bind", self._on_factory_bind, "year")
    ...
    def _on_factory_bind(self, _factory, list_item, what):
