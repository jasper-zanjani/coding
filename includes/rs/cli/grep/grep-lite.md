...
} // McNamara, Tim. *Rust in Action*. p. 68.
```

First regex (2), then a CLI framework is implemented (3).

```rs hl_lines="1 4 7-10"
use regex::Regex;
...
} // McNamara, Tim. *Rust in Action*. p. 69.
```

```rs hl_lines="2 5-12 14"
use regex::Regex;
use clap::{App,Arg};
...
} // McNamara, Tim. *Rust in Action*. p. 71-72.
```
...
} // McNamara, Tim. *Rust in Action*. p. 73.
```
...
} // McNamara, Tim. *Rust in Action*. p. 74-75.
```
...
fn main() {
    ...
    println!("{}", text);
} // McNamara, Tim. *Rust in Action*. p. 82-83.
```
...
fn main() {
    ...
    println!("{} is {} bytes long", &f.name, f_length);
    println!("{}", text);
} // McNamara, Tim. *Rust in Action*. p. 86.
```
...
fn main() {
    ...
    println!("{} is {} bytes long", &f.name, f_length);
    println!("{}", text);
} // McNamara, Tim. *Rust in Action*. p. 93-94.
```


Define a FileState enum (11). 
...
fn main() {
    ...
    println!("{} is {} bytes long", &f.name, f_length);
    println!("{}", text);
} // McNamara, Tim. *Rust in Action*. p. 97-98.
```

Implementing the Display [trait](#trait) allows us to use a standard template in a `println!` statement (12).
...
fn main() {
    println!("{:?}", f);
    println!("{}", f);
} // McNamara, Tim. *Rust in Action*. p. 101-102.
```
